...
export function bufToDate(buf: ArrayBuffer) {
    console.assert(buf.byteLength === 16, new Uint8Array(buf));
...
export function bufToInt(buf: ArrayBuffer) {
    console.assert(buf.byteLength === 4, new Uint8Array(buf)); if (buf.byteLength !== 4) debugger;
    const intBuf = new Uint32Array(buf);
    return intBuf[0];
}

export function bufToShort(buf: ArrayBuffer) {
    console.assert(buf.byteLength === 2, new Uint8Array(buf));
    const shortBuf = new Uint16Array(buf);
    return shortBuf[0];
}

export function bufToByte(buf: ArrayBuffer) {
    console.assert(buf.byteLength === 1, new Uint8Array(buf)); if (buf.byteLength !== 1) debugger;
    const byteBuf = new Uint8Array(buf);
    return byteBuf[0];
}

export function bufToFloat(buf: ArrayBuffer) {
    console.assert(buf.byteLength === 4, new Uint8Array(buf));
