...
import { bufToArray, bufTobString, bufToByte, bufTobzString, bufToDouble, bufToFloat, bufToInt, bufToShort } from "./util";
        ...
        if (record.flags & 0x8000000) {
            ...
            for (let i = 0; i < this.inventory_itemNum; ++i) {
                ...
                for (let j = 0; j < changedEntriesNum; ++j) {
                    let [newOffset, properties] = getProps(offset, buf);
                    offset = newOffset;
...
let getProps = (offset: number, buf: ArrayBuffer): [number, {
    propertiesNum: number,
    properties: {
        flag: number;
        // @TODO: More strict typing here?
